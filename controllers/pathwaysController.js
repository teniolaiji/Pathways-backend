...
const sanitiseDifficulty = (val) => {
  ...
  // Synonym map
  const map = {
    easy: "beginner", basic: "beginner", intro: "beginner",
    introductory: "beginner", introduction: "beginner",
    foundational: "beginner", foundation: "beginner",
    elementary: "beginner", starter: "beginner", novice: "beginner",
    "entry level": "beginner", "entry-level": "beginner",
    "getting started": "beginner",
    moderate: "intermediate", medium: "intermediate",
    mid: "intermediate", middle: "intermediate",
    "mid-level": "intermediate", midlevel: "intermediate",
    ...
    senior: "advanced", complex: "advanced", professional: "advanced",
  ...
  // Absolute catch-all
  console.log(`[Sanitise] Unknown difficulty "${val}" — defaulting to "beginner"`);
  return "beginner";
};
...
const sanitiseFormat = (val) => {
  ...
  const map = {
    // article
    webpage: "article", web: "article", website: "article",
    page: "article", tutorial: "article", blog: "article",
    book: "article", reading: "article", text: "article",
    documentation: "article", docs: "article", guide: "article",
    resource: "article", link: "article", post: "article",
    written: "article", reference: "article",
    "web page": "article", "blog post": "article",
    "written tutorial": "article", "tutorials and reference": "article",
    "language-specific tutorials": "article",
    // video
    podcast: "video", lecture: "video", talk: "video",
    webinar: "video", stream: "video", watch: "video",
    "video lecture": "video", "video tutorial": "video",
    "video lectures": "video", "video tutorials": "video",
    "video courses": "video", "online video": "video",
    // exercise
    project: "exercise", quiz: "exercise", practice: "exercise",
    assignment: "exercise", lab: "exercise", playground: "exercise",
    challenge: "exercise", workshop: "exercise", lesson: "exercise",
    lessons: "exercise", activity: "exercise", task: "exercise",
    coding: "exercise", "interactive lesson": "exercise",
    "interactive lessons": "exercise", "interactive learning": "exercise",
    "interactive coding": "exercise", "hands-on": "exercise",
    "hands on": "exercise", "coding exercise": "exercise",
    "practice exercise": "exercise",
    // course
    mooc: "course", certification: "course", program: "course",
    curriculum: "course", bootcamp: "course", "online course": "course",
    "free course": "course", "short course": "course",
    "learning path": "course", "learning module": "course",
...
const validateAndFixResources = async (modules, subfieldLabel, domainLabel) => {
  ...
  console.log(`[Pathway] ${totalResources - broken.length}/${totalResources} resources valid. ${broken.length} broken.`);
  ...
  const validatedReplacements = await Promise.all(
    sanitiseModules([{ resources: replacements }])[0].resources.map(async (r) => ({
      ...r,
      isValidated: await validateResourceUrl(r.url),
    }))
  );
...
const getUserPathways = async (req, res) => {
  try {
    ...
    if (domain)  pathways = pathways.filter((p) => p.assessment?.domain   === domain);
    if (subfield) pathways = pathways.filter((p) => p.assessment?.subfield === subfield);
    if (search) {
...
const getPathwayFeedbackAnalytics = async (req, res) => {
  try {
    ...
    progress.feedbackLog.forEach((entry) => {
      ...
      if (entry.feedback?.difficulty && moduleMap[key].difficulty[entry.feedback.difficulty] !== undefined) {
        moduleMap[key].difficulty[entry.feedback.difficulty]++;
      }
      if (entry.feedback?.relevance && moduleMap[key].relevance[entry.feedback.relevance] !== undefined) {
